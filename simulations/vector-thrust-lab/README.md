...
- **双构型模式**（`S.vtolMode`，UI「悬停模式」按钮切换）：
  ...
  - **B_true 在线 Jacobian 分配**（悬停模式内，UI「B_true 分配」开关）：`B = ∂M/∂[Δdw,Δδt,Δδf]` 解析 Jacobian 随工作点在线更新，增量式 `Δu = B⁻¹·I·(btrueK·(ωdes−ω) − ω̇)` 分配（对齐 Python INDI / 固件 `TandemVec_ControlAllocation.h`），把 δt↔Mz、δf↔My 交叉耦合解耦；低油门奇异回退对角映射。与定高/航向角速度指令可组合
