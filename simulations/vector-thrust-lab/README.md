...
- **双构型模式**（`S.vtolMode`，UI「悬停模式」按钮切换）：
  ...
  - **VTOL 悬停**（机头朝天，x_b 竖直）：进入时**默认关掉机翼**（`aero=false`，气动力忽略 = 无翼构型），可手动再开。控制律为误差四元数级联（姿态外环 `ωdes=−2·vtolAttKp·qe`，qe=qCmd⁻¹⊗q 机体系误差；角速度内环复用 rateKq/rateKr/rateKp，通道效率符号与 sasMode=3 一致），与固件 `flight_control.cpp` 悬停链同构。滑块语义切换为**目标指令**（四轴式）：dw→**航向角速度 ψ̇**（°/s，rate 模式松手回中停转，与固件 RATE_MODE 偏航摇杆一致）、dt→俯仰倾斜角（尾摆）、df→侧倾角（前摆）；**自稳模式下三滑块全部弹簧回中**（dt/df 松手归零→姿态回竖直）；**qCmd 跟随当前航向**（y_b 水平投影提取 ψ_est，防止航向旋转后 qe 轴错位导致倾斜控制失效/反向）。油门自动置悬停配平 `thrHover=√(m·g/2kT·wMax²)≈50%`。悬停自稳按钮（b-sas）仅区分开/关；关闭时 dt/df 直通摆角（dw 仍为角速度指令）。悬停模式只做姿态稳定，无位置闭环——倾斜指令会产生持续水平漂移，为模型固有行为
  ...
  - **B_true 在线控制分配**（悬停模式内，UI「B_true 分配」开关）：纯分配层（非 INDI）——内环输出目标力矩 `M_des = diag(I)·btrueK·(ωdes−ω)`（对角独立），分配层 `Δu = B⁻¹·(M_des − M_cur)` 转换到执行器（B = ∂M/∂[Δdw,Δδt,Δδf] 随工作点在线更新，对齐固件 `TandemVec_ControlAllocation.h`），δt↔Mz、δf↔My 交叉耦合解耦；低油门奇异回退对角映射。与定高/航向角速度指令可组合
