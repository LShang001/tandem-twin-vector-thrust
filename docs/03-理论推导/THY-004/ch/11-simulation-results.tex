...
\noindent\textbf{注意（2026-08-01 参数集变更）：}本章配平/模态/稳定性数值已按 v0.2.0 VTOL 参数集（m=0.7\,kg、kT=1.04e-5、wMax=1150、a=b=0.315\,m，28\,m/s 四方程配平）重算；积分精度（RK4 vs 欧拉）与构型无关，结论不变。图中曲线若未注明均基于旧参数集 v0.1.0 绘制。
...
\caption{配平点特征值分布（旧参数集 v0.1.0 绘制）。蓝圈：纵向（短周期SP + 长周期Ph）；红方：横航向（荷兰滚DR + 滚转收敛RR + 螺旋Sp）。}
...
\caption{配平稳定性：20秒SAS保持（旧参数集 v0.1.0 绘制）。上—前向速度 $u$，下—俯仰角 $\theta$。}
\label{fig:trim_stability}
\end{figure}

20秒配平保持（图\ref{fig:trim_stability}，基于旧参数集绘制）空速漂移$<0.1\%$；v0.2.0 参数集下油门开环呈 phugoid 慢发散（10\,s 空速衰减约$12\%$，见图\ref{fig:trim_stability}后述文本与回归基线），需油门反馈环方可维持配平速度。
