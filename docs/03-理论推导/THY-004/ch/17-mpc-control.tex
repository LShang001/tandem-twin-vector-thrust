...
\noindent\textbf{本章要点：}在悬停构型（机头朝天，$x_b$=推力轴=竖直）下，给出模型预测控制（MPC\cite{rawlings2020model}）从理论形式化、线性化设计、非线性扩展、在线辨识到嵌入式求解的完整算法体系，并给出与 SAS/INDI/级联/LQR 的仿真对比实验。全部数值基于 \texttt{scripts/mpc\_proto.py}、\texttt{mpc\_adv.py}、\texttt{ident\_probe.py} 可复现。
...
\textbf{快速梯度法（FGM）}：投影梯度法收敛率为 $O(1/k)$（次线性），
可用 Nesterov 动量加速至 $O(1/k^2)$（Beck--Teboulle 形式）：
...
即"梯度步 + 外推动量"。该加速对框约束 QP 保持收敛性\cite{mayne2000constrained}。
实测收敛轨迹见图\ref{fig:mpc_fgm}：FGM-25 的目标函数余量
（$6.7\times10^{-3}$）低于同迭代 PGD-25（$8.4\times10^{-3}$），且仅需
PGD-400 的 $1/16$ 计算量。嵌入式 C 基准（STM32H743 同构，$g++$\,-O2，
6DOF 全动力学闭环：电机滞后+平动+陀螺）见表\ref{tab:mpc_embedded}：
FGM-10 单步 $0.019$\,ms——$200$\,Hz 控制周期仅占 $0.38\%$ CPU，内存 46.6\,KB
（float）。该余量为执行器/滤波器/通信留出充足预算。
...
NMPC 的预测模型越完整，对参数失配的鲁棒性越强（见\ref{sec:mpc_sim}节实验）：
...
\textbf{悬停点激励辨识}——两者互补：RLS 提供慢趋势，激励辨识提供
精确 $\mathbf{B}_{\mathrm{eff}}$。
本章验证以下辨识理论：
...
用于第\ref{sec:mpc_adapt}节自适应验证；其余为名义参数）。三场景收敛对比见图\ref{fig:mpc_scenarios}，
...
\caption{FGM vs PGD 收敛轨迹（$N=30$ 展开式 QP，目标函数余量）。
FGM-25 余量 $6.7\times10^{-3}$ 低于 PGD-25 的 $8.4\times10^{-3}$；
FGM-10 单步 $0.019$\,ms（STM32H743 同构）。}
\label{fig:mpc_fgm}
\end{figure}

\begin{table}[H]
\centering
\caption{嵌入式 LMPC 求解器基准（STM32H743 @480MHz 同构 g++ -O2，
6DOF 全动力学闭环：电机滞后+平动+陀螺；5° 扰动 3\,s）}
\label{tab:mpc_embedded}
\small
\begin{tabular}{l c c c c}
\toprule
\textbf{求解器} & \textbf{迭代} & \textbf{单步耗时} & \textbf{@200Hz 占用} & \textbf{eps 末值} \\
\midrule
...
\midrule
SAS（PD+阻尼） & 增益映射 & 后验限幅 & $\sim\mu$s & 中 \\
INDI（在线 B） & 在线 Jacobian & 增量限幅 & 10--50$\mu$s & 需惯量修正 \\
级联 P+在线 B & 在线 Jacobian & 增量限幅 & $\sim$10$\mu$s & 好（耦合最优） \\
LQR & 悬停点线性化 & 无 & $\sim\mu$s & 好（状态反馈） \\
LMPC & 线性+滞后增广 & \textbf{显式预测} & \textbf{0.02--0.05 ms（FGM）} & \textbf{好（反馈鲁棒）} \\
NMPC & \textbf{非线性全模型} & \textbf{显式预测} & 10--30 ms（原型） & \textbf{最优（实验）} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{结论与工程路径}：NMPC 在约束活跃与过渡场景显著优于传统方法（60° 扰动
末值误差：名义 NMPC $0.0052$、自适应 NMPC $0.0046$，均优于 SAS 的 $0.0303$，
约一个数量级）；其代价是求解实时性与模型依赖。
...
（acados\cite{verschueren2022acados}），先从仿真侧过渡轨迹研究开始，台架标定后逐步上实机。
