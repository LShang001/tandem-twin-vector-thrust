...
\begin{equation}
    ...
    \qquad
    t_{k+1} = \tfrac{1}{2}\Big(1+\sqrt{1+4t_k^2}\Big),
    \quad
...
即"梯度步 + 外推动量"。实测收敛轨迹见图\ref{fig:mpc_fgm}：FGM-25 的目标函数余量
...
LMPC & 线性+滞后增广 & \textbf{显式预测} & \textbf{0.02--0.05\,ms（FGM，实测）} & \textbf{好（反馈鲁棒）} \\
