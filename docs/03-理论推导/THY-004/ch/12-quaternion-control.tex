...
\begin{equation}
    ...
    \label{eq:eta_dot}
\end{equation}
（由四元数运动学$\dot{\mathbf{q}} = \frac{1}{2}\mathbf{q}\otimes[0,\boldsymbol{\omega}]^T$和误差四元数定义导出；相应矢量部分为$\dot{\boldsymbol{\varepsilon}}_{\text{err}}=\frac{1}{2}(\eta_{\text{err}}\boldsymbol{\omega}+\boldsymbol{\varepsilon}_{\text{err}}\times\boldsymbol{\omega})$，在$\eta_{\text{err}}\approx1$的小误差邻域退化为式\ref{eq:lqr_eps_dot}的线性化运动学$\dot{\boldsymbol{\varepsilon}}_{\text{err}}\approx\frac{1}{2}\boldsymbol{\omega}$）。因此
\begin{align}
...
$\mathbf{q}_{\text{err}}=\mathbf{q}_{\text{cmd}}^{-1}\otimes\mathbf{q}$严格表达在
$\mathbf{q}_{\text{cmd}}$ 系，仅当 $\mathbf{q}_{\text{cmd}}\approx\mathbf{q}$（误差小）时一阶等价机体系。
...
保证 $\mathbf{q}_{\text{err}}$ 恒小。实机调试中悬停航向指令 $19^\circ$ 不收敛即由此定位（2026-08-04）。
...
\begin{equation}
    ...
    \label{eq:slerp}
\end{equation}
其中$\tau = (t - t_0)/(t_1 - t_0) \in [0,1]$为归一化过渡进度，$\Omega = \arccos(|\mathbf{q}_{\text{from}}\cdot\mathbf{q}_{\text{to}}|)$（点积为负时先对$\mathbf{q}_{\text{to}}$取反，沿短弧插值）为两四元数间的角度。slerp保证过渡路径在$S^3$单位球面上沿大圆弧最短路径行进，无角速率突变，且任意中间态均为有效单位四元数——这一性质在欧拉角域无等价实现。
...
\end{equation}
其中 $\boldsymbol{\varepsilon}_{\text{err}} = [\varepsilon_x,\ \varepsilon_y,\ \varepsilon_z]^T$
为误差四元数的矢量部分（下标对应体轴分量）。在手工整定实现
（\texttt{controllers.py} 的 \texttt{QuatSASController}）中，该映射采用
$\mathbf{B}$ 的主通道对角近似（$\Delta\omega=\tau_x/(-2\tau_0)$、
$\delta_t=\tau_y/(-bT_0)$、$\delta_f=\tau_z/(aT_0)$），与本章正文给出的完整
$\mathbf{B}_{\text{true}}^{-1}$ 不同——对角近似丢弃反扭矩交叉项换取零求逆开销，
第\ref{sec:lqr}节的 LQR 则完整利用交叉项，这正是表\ref{tab:ctrl_compare}中
三者性能梯度的来源之一。
...
基准律式(\ref{eq:quat_pd})在定点悬停附近表现良好，但tail-sitter垂直起飞等$90^\circ$级机动暴露了三个工程问题，
相应的改进均已在仿真实现（\texttt{simulations/}\allowbreak\texttt{high-fidelity-analysis/}\allowbreak\texttt{controllers.py}）中验证：

\textbf{(1) 阻尼增益调度}。基准律的角速率阻尼按悬停小信号整定（俯仰通道阻尼比$\zeta\approx0.22$），大角度机动中振荡严重。引入随误差幅值调度的阻尼因子
\begin{equation}
    \boldsymbol{\tau} = -\mathbf{K}_q\boldsymbol{\varepsilon}_{\text{err}} - g(\boldsymbol{\varepsilon}_{\text{err}})\,\mathbf{K}_\omega\left(\boldsymbol{\omega} - \boldsymbol{\omega}_{\text{ref}}\right),
    \qquad
    g = 1 + 3\min\!\left(\frac{\|\boldsymbol{\varepsilon}_{\text{err}}\|}{0.15},\ 1\right)
    \label{eq:quat_pd_sched}
\end{equation}
$\|\boldsymbol{\varepsilon}_{\text{err}}\|\geq0.15$（约$17^\circ$）时$g=4$（$\zeta\approx0.88$），悬停点$g=1$恢复基准律。由于$g(\cdot)\geq1$有界且仅依赖状态，Lyapunov分析中$\dot{V}=-g\,\boldsymbol{\omega}^T\mathbf{K}_\omega\boldsymbol{\omega}\leq0$仍然成立，稳定性结论不受影响。
...
\textbf{(3) 条件积分抗饱和}。机动期间大姿态误差会使积分项饱和，过渡结束后饱和积分缓慢放电、形成持续力矩偏置（仿真中表现为过渡后姿态缓慢爬离目标）。改为仅当$\|\boldsymbol{\varepsilon}_{\text{err}}\|<0.2$时才积分，机动期间冻结积分器，消除了该伪稳态偏差。
