...
\end{equation}
其中$\boldsymbol{\varepsilon}_{\text{err}} = [\varepsilon_{\text{err},1}, \varepsilon_{\text{err},2}, \varepsilon_{\text{err},3}]^T$为误差四元数的矢量部分。在手工整定实现（\texttt{controllers.py}的\texttt{QuatSASController}）中，该映射采用$\mathbf{B}$的主通道对角近似（$\Delta\omega=\tau_x/(-2\tau_0)$、$\delta_t=\tau_y/(-bT_0)$、$\delta_f=\tau_z/(aT_0)$），与本章正文给出的完整$\mathbf{B}_{\text{true}}^{-1}$不同——对角近似丢弃反扭矩交叉项换取零求逆开销，第\ref{sec:lqr}节的LQR则完整利用交叉项，这正是表\ref{tab:ctrl_compare}中三者性能梯度的来源之一。
...
基准律式(\ref{eq:quat_pd})在定点悬停附近表现良好，但tail-sitter垂直起飞等$90^\circ$级机动暴露了三个工程问题，相应的改进均已在仿真实现（\texttt{simulations/high-fidelity-analysis/controllers.py}）中验证：
