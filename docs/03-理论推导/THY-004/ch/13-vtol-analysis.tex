...
\begin{itemize}
    \item \textbf{通道语义按悬停构型重读}：在机体坐标系中，前电机摆座绕$z_b$轴摆动（\textbf{上摆=滚转主控}）在悬停构型下产生绕横轴的侧倾力矩；尾电机摆座绕$y_b$轴摆动（\textbf{下摆=俯仰主控}）仍为俯仰。两摆座控制功能与固定翼模式一致，无需重新分配（通道命名见第\ref{sec:axis_semantics}节）。
...
图\ref{fig:vtol_hover}展示了悬停姿态的闭环保持性能。在精确配平转速（式\ref{eq:hover_omega0}，$\omega_0\approx574.0$\,rad/s，v0.2.0 参数集）和四元数SAS的PD反馈下，俯仰角$\theta$稳定保持在$-90^\circ$（仿真内未见可分辨偏差），8\,s内高度漂移为零。该稳定性验证了误差四元数控制律（式\ref{eq:quat_pd}）在万向锁奇异点附近的有效性——欧拉角SAS在此工况下无法正常工作。
...
\begin{enumerate}[label=(\arabic*)]
    \item \textbf{油门需求高}：按上节对称悬停解，悬停需约$49.9\%$油门（$\omega_{0,\text{hover}}\approx574$\,rad/s，v0.2.0 参数集），推力裕度约$50\%$用于机动控制；早期文档中"84.6\% 油门"为旧参数集残留值，已废弃。以当前 MODEL-DEFAULT（未标定）参数，该构型的推重比在悬停点偏弱。
