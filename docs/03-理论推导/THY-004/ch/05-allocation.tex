...
当前MODEL-DEFAULT参数给出$k_Q/(k_Tb)\approx 0.086$（$k_Q/k_T\approx0.027$、$b=0.315$\,m，2026-08-09 实机三轮调参锚定；早期 0.117 为旧参数残留）；该比值未经完整台架标定，
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item \textbf{非线性精确映射}：当计算资源允许时，直接使用非线性映射（式\ref{eq:six_component}）
          ...
          此方法在摆角$>15^\circ$的大机动中是必要的，因为$\sin\delta \approx \delta$
          的近似精度开始退化（$15^\circ$时$\sin\delta/\delta \approx 0.989$，
          误差约$1.1\%$；$25^\circ$时约$3.1\%$；$45^\circ$时已达$10\%$）。
\end{enumerate}

\subsection{低油门差速控制退化与增益调度}

差速效能$\partial M_x / \partial \Delta\omega = -2 k_Q \omega_0^2 \propto T_0$——
\textbf{差速控制力矩的大小与总推力成正比}（悬停构型语义下该通道为偏航主控）。这是差速反扭力矩机制的物理固有特性，
...
或引入气动副翼作为低油门的差速控制补充（但这增加了构型复杂度）。

\textbf{增益调度与瞬态失配（2026-08-09 实机三层定位）}：有效回路增益恒为
$k_p\cdot\left(w_{0,\text{actual}}/w_{0,\text{used}}\right)^2$——分配器与力矩
均按转速平方缩放，油门变化即增益变化。三层坑与修复：
① 低油门 $\Delta\omega$ 指令按 $1/w_0^2$ 归一化时爆炸（调度改为 $(w_0/w_h)^2$）；
② 物理力矩仍 $\propto w_0^2$，稳态有效增益随油门放大（调度封顶 1.0）；
③ \textbf{瞬态失配才是抖油门震荡根因}——分配器用指令油门而力矩用实际转速（电机 $\tau_m$ 滞后），
油门释放瞬间 $w_{0,\text{actual}}/w_{0,\text{cmd}}\approx1.33$，有效增益瞬态越过震荡点约 160\,ms。
修复范式：$\tau_m$ 一阶观测器 $w_{\text{est}} \mathrel{+}= (w_{\text{cmd}}-w_{\text{est}})\cdot\Delta t/\tau_m$，
B 矩阵工作点、调度与增量状态全部改用观测转速（$w_{0,\text{eff}}$，见第\ref{sec:vtol}节实机锚点）。
...
从$3n$降至$3$（恰等于控制目标维度），使$\bm{B}$方阵且对角——
