...
\begin{tikzpicture}[>=Stealth,
  ...
  \begin{scope}[shift={(0,0)}]
    ...
    \draw[axis] (-3.0,0) -- (3.25,0) node[annot, right=1pt] {$x'$};
    \draw[axis] (0,-1.55) -- (0,1.8) node[annot, above=1pt] {$y'$};
    \fill (0,0) circle (2pt); \node[annot, anchor=north west] at (.08,-.06) {CG};
    \node[annot] at (-2.25,.85) {$\odot\,z'$};
    ...
    \draw[spin] (-1.65,-1.42) -- (-2.45,-1.42)
      node[annot, below=1pt, text=purple!70!black] {$\bm h_t\parallel -x'$};
    \draw[spin] (1.85,-1.42) -- (2.65,-1.42)
      node[annot, below=1pt, text=purple!70!black] {$\bm h_f\parallel +x'$};
    ...
    \node[annot, text=teal!70!black] at (-.7,1.48) {$M_{z'}^{(T)}=aT_f\sin\delta_f$};

    \draw[dim] (-1.65,-1.05) -- node[annot] {$b$} (0,-1.05);
    \draw[dim] (0,-1.05) -- node[annot] {$a$} (1.85,-1.05);
    \node[font=\footnotesize\sffamily] at (0,-2.05) {(a) $\{P\}$系：前摆产生$M_{z'}$主项};
  \end{scope}
  ...
  \begin{scope}[shift={(7.4,0)}]
    \draw[body] (-2.7,-1.25) rectangle (2.7,1.25);
    \draw[axis] (-3.0,0) -- (3.25,0) node[annot, right=1pt] {$x'$};
    \draw[axis] (0,1.8) -- (0,-1.65) node[annot, below=1pt] {$z'$};
    \fill (0,0) circle (2pt); \node[annot, anchor=north west] at (.08,-.06) {CG};
    \node[annot, align=center] at (2.25,.92) {$\odot\,y'$\\[-1pt]\scriptsize 视向 $-y'$};
    ...
    \node[annot, text=teal!70!black] at (.2,1.48) {$M_{y'}^{(T)}=-bT_t\sin\delta_t$};
    \draw[dim] (-1.65,-1.05) -- node[annot] {$b$} (0,-1.05);
    \draw[dim] (0,-1.05) -- node[annot] {$a$} (1.85,-1.05);
    \node[font=\footnotesize\sffamily] at (0,-2.05) {(b) $\{P\}$系：尾摆产生$M_{y'}$主项};
  \end{scope}
  \node[annot, text=orange!85!black] at (3.7,-2.55)
    {模型轴向力矩：$M_{x'}=-Q_f\cos\delta_f+Q_t\cos\delta_t$};
\end{tikzpicture}%
}
\caption{推进模型系中的推力矢量映射。$M_{z'}^{(T)}$ 与 $M_{y'}^{(T)}$
是推力力臂主项；完整映射还含 $-Q_t\sin\delta_t$ 与
$-Q_f\sin\delta_f$ 的反扭矩投影。零摆角时两台推力均沿 $+x'$，
反向旋转使等转速轴向反扭矩抵消。图示模型轴不直接等同于固件操纵通道。}
