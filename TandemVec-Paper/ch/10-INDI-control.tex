...
唯一仍需局部建模的是角加速度效能矩阵$\hat{\mathcal B}$。
若第\ref{sec:cascade_Btrue}节的$\mathbf B_{\mathrm{true}}$定义为力矩对执行器的Jacobian，
则两者满足$\mathcal B=\mathbf I^{-1}\mathbf B_{\mathrm{true}}$，不能直接等同。

\subsection{INDI 的效能矩阵依赖}

本平台可从力矩Jacobian$\mathbf B_{\mathrm{true}}(\mathbf u_k)$构造
$\mathcal B_k=\mathbf I^{-1}\mathbf B_{\mathrm{true}}$。冻结矩阵与在线矩阵的差异取决于
摆角、差速和转速工作点；早期“误差超过50\%”未绑定当前可重复工况，故不再作为必然结论。
...
\begin{equation}
    ...
    +
    \mathbf{B}_{\mathrm{true}}^{-1}(\mathbf{u}_k)\,\mathbf I
    \cdot
    \left(
...
其中虚拟控制量$\boldsymbol\nu$和$\dot{\boldsymbol\omega}_{\mathrm{IMU}}$的单位均为角加速度，
故差值必须先乘$\mathbf I$变为力矩增量，再经力矩Jacobian逆得到执行器增量。

\textbf{非奇异性边界}：零摆角矩阵在$T_0,Q_0,a,b>0$时局部满秩；
有限摆角的完整$3\times3$矩阵不能仅由右下$2\times2$子块证明全域非奇异。
工程实现应在限定工作域扫描最小奇异值或条件数，并在低油门与饱和附近退化处理。

\subsubsection{虚拟控制量 $\nu$ 的生成}

以下外环保留历史Web原型的欧拉SAS结构，用于说明INDI接口；当前固件滚转/俯仰使用四元数短弧，
偏航使用ratchet hold，不能把下式写成固件实现：
\begin{align}
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item \textbf{解析求逆}：计算$\mathbf{B}_{\mathrm{true}}^{-1}$，并采用归一化条件数或最小奇异值
          判断病态；固定$10^{-6}$行列式阈值具有单位和尺度依赖，不能直接作为固件规则；
...
历史欧拉外环的符号必须与项目显示角约定（$\dot\theta=-q$、$\dot\phi=-p$）自洽。
早期Python脚本曾出现符号翻转后的发散轨迹，但该脚本与固件四元数外环并不同构；
它只能作为接口符号单元测试，不能作为当前闭环性能证据。

内环带宽$K^{\text{rate}}$会同时改变响应速度、噪声放大和执行器增量。
历史脚本关于“恢复时间缩短60\%”和“12\,s漂移小于0.1\%”尚未在统一植物、限幅和随机种子下复验，
因此保留为待重现实验假设，不作为本章结论。

\subsection{与Web基线SAS的概念对比}

\begin{table}[H]
\centering
\caption{Web基线SAS与INDI原型的控制律概念对比}
\label{tab:sas_vs_indi}
\setlength{\tabcolsep}{4pt}
\small
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{2.8cm} *{2}{>{\raggedright\arraybackslash}X}}
\toprule
\textbf{维度} & \textbf{Web基线SAS（绝对指令）} & \textbf{INDI原型（增量指令）} \\
\midrule
...
\textbf{切换逻辑边界}：早期15\%阈值没有物理标定来源。当前固件采用
$0.6\,w_{\mathrm{hover}}$的矩阵工作点下限和5\%零油门归中门控，但尚未实现INDI模式切换；
若未来加入，应根据条件数、可用力矩和台架数据定义阈值。
...
\end{equation}
其中$\boldsymbol{\omega}_{\text{err}}$为当前角速率误差（反映了机动剧烈程度），$\sigma_\omega$为归一化尺度参数。快速机动时$\omega_c$自动提高以捕获高频动态，稳态时$\omega_c$降至最低以抑制噪声。Jiang等报告的仿真在其模型与工况下显示，该方案在估计误差、稳定性和瞬态响应指标上优于所比较的固定增益互补滤波\cite{jiang2025adaptive}；该结论尚未在本文统一植物上复验。
...
Lude\~{n}a Cervantes等（2020）\cite{ludena2020flight}将固定滞后平滑器用于INDI的角加速度估计。其状态向量包含角速率和角加速度：
\begin{equation}
    \mathbf{x} = [\phi, \theta, p, q, r, \dot{p}, \dot{q}, \dot{r}]^T
    \label{eq:fixedlag_state}
\end{equation}
状态方程以刚体转动动力学为过程模型，观测方程为IMU输出。固定滞后$N$会引入
$N\Delta t$额外延迟；采样间隔小并不能自动证明这段延迟不破坏INDI的执行器--测量同步，
必须用闭环相位裕度和时序对齐试验选择$N$。
...
MEMS角加速度计的潜在优势是避免对陀螺信号作数值微分；其传感器带宽、相位延迟和本底噪声
仍为非零，输出也需经过标定与时序同步后才能作为INDI的$\dot{\mathbf y}_0$。
商用器件带宽与旋翼振动环境之间仍需独立评估。
...
\midrule
后向欧拉（\ref{eq:bwd_euler}） & 一采样差分 & 无 & 无 & 极小 & 概念仿真 \\
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item \textbf{HIL验证阶段}：由Allan方差和实测延迟建立IMU模型，在统一闭环中比较候选估计器；
    \item \textbf{实机飞行阶段}：只有在HIL证明噪声、相位和执行器同步满足要求后，
          才选择软件估计器或专用角加速度传感器。
\end{enumerate}
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item \textbf{效能矩阵的固定规模}：$\mathbf B_{\mathrm{full}}$近对角但并非对角，
          固件仍在线构造稠密$3\times3$矩阵并解析求逆；优势是固定计算量，不是避免求逆；
    \item \textbf{外环可替换性}：历史原型保留Web欧拉SAS，正式固件适配应改用四元数短弧与
          ratchet hold；增量内环并不要求绑定某一种外环。
\end{enumerate}
此外，增量控制思想已被扩展到滑模容错飞控\cite{wang2019incremental}，说明故障场景下可把
效能变化纳入增量补偿；但该结论依赖故障检测、剩余控制可达性和执行器冗余。
本构型单发失效后输入秩与总推力同时变化，不能仅凭增量形式宣称具备容错能力。
