...
约定细节（确保符号的一致性）：
\begin{itemize}
    \item 前电机的轴线与机体纵轴重合，机体受力方向沿$+x_b$；前转子角动量方向沿$+x_b$；
          旋转矢量方向沿$+x_b$；
    \item 尾电机同样与机体纵轴共线，机体受力方向沿$+x_b$；尾转子反向旋转，
          零摆角时角动量方向沿$-x_b$，与机体受力方向相反；
    ...
    \item 摆角符号约定：前电机绕$+z_b$的右手旋转为正（$\delta_f > 0$表示推力向右偏）；
          尾电机绕$+y_b$的右手旋转为正（$\delta_t > 0$表示机体受力向$-z_b$偏，即向上；排气向下）。
\end{itemize}
...
\begin{itemize}
    ...
    \item $M_{t,y} = -b T_t \sin\delta_t$：推力在机体上的$z$向分量（$-T_t\sin\delta_t$，
          ...
          注意：$\delta_t>0$时尾推力受力向上，抬升尾部并产生低头力矩；NED右手系中$M_y<0$为机头向下；
    \item $M_{t,z} = -\tau_t \sin\delta_t$：尾摆角使反扭矩在$z$方向产生投影——
          俯仰通道（尾摆）向偏航通道的一阶小角度耦合。
\end{itemize}
...
\begin{itemize}
    ...
    \item $\delta_t > 0$（尾摆下偏——排气向下、机体受力向上）：
          ...
          $M_y \approx -b T_t \delta_t < 0$（低头力矩），符合尾部向上受力的俯仰直觉；
