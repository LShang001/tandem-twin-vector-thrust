...
推荐整定：$\omega_c=4$\,rad/s，$\omega_o=8$\,rad/s（满足下文$\omega_c\leq\omega_o/2$准则）。
...
固件默认$\omega_o=8$、$\omega_c=6$略超$\omega_o/2$准则（工程折中，实机以稳定性裕度验证为准）。
...
\textbf{实机辨识首测结果（2026-08-09）}：角速度模式方波激励下测得
$b\approx 0.35/0.36/0.90$（roll/pitch/yaw）、$d_{\text{yaw}}\approx -7.36$——
yaw 通道扰动为负值且量级显著，对应\textbf{前后电机不对称约 17\%}（ESC 校准前的
推力/反扭差异，与推进系统标定直接相关）；
差速/反扭矩比经三轮调参收敛于 $k_Q/k_T\approx 0.027$（0.030$\to$0.025$\to$0.027），
为第\ref{sec:mapping}章反扭矩模型提供实机量级锚定。
注意：$b$ 为激励频率下的集总测量（含执行器 $\tau_m$ 滞后衰减），
不可直接反推惯量（$I_{\text{actual}}=I_{\text{nom}}/b$ 仅执行器理想时成立）。
...
按辨识定义$\hat{b}=I_{\mathrm{nom}}/I_{\mathrm{true}}$（固件约定），真实惯量$I_{\mathrm{true}}=I_{\mathrm{nom}}/\hat{b}$；
将增益取为$K_p/\sqrt{\hat{b}}$使带宽按$1/\sqrt{\hat{b}}=\sqrt{I_{\mathrm{true}}/I_{\mathrm{nom}}}$缩放——
