...
\begin{equation}
    ...
    \label{eq:sas_pitch}
\end{equation}
其中$\delta_{t0}=-18.702^\circ$为28\,m/s四方程配平偏置（v0.2.0 参数集），$\theta_0=-\alpha_0=-4.122^\circ$
...
Rate Command 模式（第\ref{eq:rate_command}节）的操纵手感完全由"杆量到目标角速率"的映射决定。
本构型实机采用穿越机（Betaflight）风格的\textbf{三参数模型}，曲线函数见 \texttt{include/RcCurve.h}：
\begin{equation}
    \underbrace{\text{rcCmd} = \text{rc}\left[\text{expo}\cdot\text{rc}^2 + (1-\text{expo})\right]}_{\text{expo: input center curve}}
    \qquad
    \underbrace{\omega_{\text{ref}} = \frac{\text{rcCmd}\cdot\text{rcRate}\cdot 200}{1 - |\text{rcCmd}|\cdot\text{super}}}_{\text{rcRate: gain, super: edge curve}}
    \label{eq:rc_curve}
\end{equation}
...
3/4 杆后急剧上扬至满杆 333°/s（暴力翻滚）——
这正是穿越机"细腻中心 + 暴力边缘"手感的数学来源。
...
\textbf{实机限幅解耦教训}：初版 RATE\_MODE 限幅复用姿态外环的 80°/s
（$k_{\text{MaxTargetRate}}$），曲线在约半杆（默认参数 57°/s）即被削平——
满杆暴力手感完全被限幅吞掉。2026-08-11 将 RATE\_MODE 限幅独立为 600°/s
（默认曲线理论值 333°/s 之上留余量）：限幅从"姿态环约束"解耦为"曲线之上的安全网"——
rcRate 上调到 1.0 时满杆 667°/s 触限兜底，
但 super 双曲在 rcRate 极端（如 2.5）时理论指令可达数千 °/s，限幅不可完全移除。
实机注意：满杆角速率从存档验证的 80°/s 提升到 333°/s（4 倍以上），
调试须从低杆量逐步体验（曲线中心区保持细腻）。
