...
基于第\ref{sec:dynamics}节的RK4高精度积分框架和第\ref{sec:INDI}节的INDI控制律方案，使用Python独立实现的六自由度仿真程序（\texttt{simulations/high-fidelity-analysis/}），对配平状态、线性化模态、积分精度和闭环响应进行定量分析与可视化。

\noindent\textbf{注意（2026-08-01 参数集变更）：}本章配平/模态/稳定性数值与图（配平表、模态表、特征值分布、SAS 阶跃、配平保持）均已按 v0.2.0 VTOL 参数集（m=0.7\,kg、kT=1.04e-5、wMax=1150、a=b=0.315\,m，28\,m/s 四方程配平）重算重绘；积分精度（RK4 vs 欧拉）与构型无关，结论不变。其余章节（VTOL 控制、级联/INDI）的图仍基于旧参数集 v0.1.0 绘制。

\subsection{纵向配平}
\label{sec:trim}

在$V=28$\,m/s水平定直飞行条件下，Newton法联立求解纵向三自由度配平（v0.2.0 VTOL 参数集）：

\input{sim-data/trim_table.tex}

该配平点（$\alpha=4.122^\circ$、$\delta_{t0}=-18.702^\circ$、$\delta_{f0}=-1.739^\circ$、油门$19.2\%$）由Web JS仿真四方程（Fx/Fz/My/Mz）求解（\texttt{scripts/trim\_solve.py}）；Python独立实现的简化气动模型（三方程、$\delta_f=0$）给出$\alpha=4.123^\circ$、$\delta_{t0}=-12.667^\circ$、油门$23.4\%$，差异来自前摆反扭耦合与气动投影细节。
...
短周期模态（$f\approx1.03$\,Hz，$\zeta\approx0.52$）由$C_{m\alpha}$和$C_{mq}$支配，具备良好快速衰减（$\tau\approx0.30$\,s）。长周期（phugoid）特征值$+0.0177$为\textbf{正实根——无油门反馈下慢发散}（发散时间常数约$56$\,s），与Web仿真10\,s内空速衰减约$12\%$一致；这是固定油门开环的固有行为，需油门/速度反馈环改善（SAS仅覆盖姿态）。横航向：滚转收敛极快（$\tau\approx21$\,ms，小惯量$I_x$），荷兰滚（$f\approx1.12$\,Hz，$\zeta\approx0.20$）阻尼偏弱，螺旋模态缓慢收敛（$\tau\approx6.4$\,s）。
...
$t=2$\,s时施加俯仰角速率脉冲（$\Delta q=5^\circ$/s），角速率在约0.5\,s内被阻尼衰减（图\ref{fig:sas_response}），姿态偏差峰值约$0.7^\circ$；$\delta_t$修正峰值约$0.2^\circ$（相对配平基准$-18.7^\circ$），远低于$\pm25^\circ$限幅。姿态缓慢回归配平（姿态环带宽受$k_{\theta}=0.3$限制），验证了角速率阻尼+姿态PI反馈的有效性。

\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{fig-sim/trim_stability.pdf}
\caption{配平保持（油门开环，v0.2.0 参数集）：20秒内空速衰减约$30\%$——phugoid 慢发散的可视化（特征值$+0.0177$），需油门反馈环维持配平速度；俯仰角偏差保持在$1^\circ$以内。}
\label{fig:trim_stability}
\end{figure}

20秒配平保持（图\ref{fig:trim_stability}）：空速呈 phugoid 慢发散（20\,s 内衰减约$30\%$，10\,s 约$12\%$，与 Web JS 回归基线一致），俯仰角由 SAS 保持在配平值$1^\circ$以内——速度发散需油门反馈环方可抑制，姿态增稳正常工作。

\subsection{INDI控制器初步验证}

\textit{（本小节 INDI 已按 v0.2.0 参数集重跑，并修复了分配中缺失的惯量逆 $\mathbf{I}$——角加速度指令必须先乘惯量转换为力矩增量再经 $\mathbf{B}^{-1}$ 分配，否则各向异性惯量使滚转通道发散。2026-08-05 转子角动量完整化（含摆角投影的三轴 $\mathbf{h}$，与 \texttt{dynamics.mjs} 对齐）后重跑：俯仰改善 $76.4\%\rightarrow76.3\%$、滚转 $98.4\%\rightarrow91.7\%$——滚转通道对陀螺/进动项更敏感。）}基于在线Jacobian $\mathbf{B}_{\mathrm{true}}$（式\ref{eq:B_true}）的INDI控制器在零扰动配平保持中表现与SAS相当（12秒速度漂移$<0.1\%$），验证了增量控制架构的配平稳定性。在扰动抑制对比中，修复惯量逆后的 INDI 累积误差相对 SAS 改善 $76.3\%$（俯仰）与 $91.7\%$（滚转）。在角速率扰动响应中，INDI表现出比SAS更大幅度的执行器调节——这与INDI理论预测一致：增量控制通过$\mathbf{B}^{-1}$直接映射角加速度误差至执行器增量，对扰动的初始响应更激进。然而，在$\dot{\boldsymbol{\omega}}$估计仅依赖后向欧拉差分的纯传感器方案下，噪声放大效应导致INDI扰动抑制性能不如结构更保守的SAS。这一发现突显了第\ref{sec:INDI}节指出的角加速度滤波（§\ref{sec:accel_estimation}）对INDI实际性能的决定性影响。
...
\textbf{实验设置}：飞行器处于28\,m/s巡航配平状态（24\,m/s 配平尾摆需 $-40.5^\circ$ 超限，不可达），$t=2$\,s时在前摆（上摆，悬停语义下为滚转通道）施加$\delta_f=10^\circ$阶跃，产生俯仰耦合扰动$-\tau_f\sin 10^\circ$。控制器需保持其余通道（俯仰/滚转/偏航角速率为零），测量俯仰角$\theta$偏离配平的峰值作为耦合抑制指标。陀螺噪声取标准差$0.002$\,rad/s的典型MEMS量级，每配置进行5次蒙特卡洛仿真取均值。INDI的角加速度估计采用后向欧拉差分。
...
上述数值结果对气动导数不确定性敏感。以短周期模态频率为例，若$C_{m\alpha}$偏离MODEL-DEFAULT值$\pm30\%$，短周期频率按$\sqrt{1\pm0.3}$缩放，从1.03\,Hz移至0.86--1.19\,Hz，阻尼比相应变为0.44--0.60（按同缩放近似）。该量级变化在常规SAS增益补偿能力之内，但突显了台架标定和CFD验证对飞行品质定量评估的必要性。

\subsection{实机初步验证（2026-08）}
本文写作时（v0.2.0 参数集，MODEL-DEFAULT 未标定）仿真结论仍属概念级；
2026-08 起项目进入实机首飞验证阶段，以下成果为本章结论提供了初步锚点：
\begin{itemize}
    \item \textbf{执行器轴语义实机锚定}：手动模式摇杆直通验证——roll\_raw$\to$上摆（前摆）、
          pitch\_raw$\to$下摆（尾摆），确认悬停构型语义（第\ref{sec:axis_semantics}节）；
    \item \textbf{kQ 实机标定}：差速/反扭矩比经三轮调参收敛于 $k_Q/k_T\approx0.027$
          （0.030$\to$0.025$\to$0.027），支撑第\ref{sec:mapping}章反扭矩模型的量级锚定；
    \item \textbf{差速增益调度与 $\tau_m$ 观测器}：抖油门震荡三层定位（第\ref{sec:allocation}章增益调度段），
          实机验证有效增益公式与观测器修复的有效性；
    \item \textbf{滤波滞后边界}：物理减震底座下数字滤波近全关（滞后 $4.2^\circ$@1Hz 稳 / $6.9^\circ$ 振铃），
          与第\ref{sec:accel_estimation}节角加速度滤波的群延迟讨论互相印证；
    \item \textbf{在线辨识首测}：角速度模式方波激励测得 $b=0.35/0.36/0.90$、
          $d_{\text{yaw}}=-7.36$（电机不对称 17\%），验证第\ref{sec:online_id}节 RLS 辨识链路。
\end{itemize}
上述实机数据全部来自非标定状态（MODEL-DEFAULT 参数 + 实机逐轮调整），
其意义在于验证本文理论框架的符号/结构正确性，而非定量预测飞行品质。
