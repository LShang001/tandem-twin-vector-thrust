...
基于 STM32H743 的纵列双发正交单轴矢量推力 + 差速反扭的固定翼/VTOL 飞控固件（**机体轴序：FRD，x_b=机身长轴=推力轴**）。前电机(CW)绕 z_b 摆动 → **偏航**主控（Mz），尾电机(CCW)绕 y_b 摆动 → **俯仰**主控（My），**差速 Δω 绕推力轴（x_b）反扭**：水平巡航时 = 滚转（Mx），**垂直悬停（机头朝天、x_b 竖直）时 = 世界航向**——同一执行器在不同姿态对应不同世界轴，这正是四元数姿态控制的意义。物理逆解层（I×α→M_cmd→BTRUE 在线 Jacobian 控制分配）将上层纯运动学控制器与具体飞行器模型解耦。
