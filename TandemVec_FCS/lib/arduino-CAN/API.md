...
> 项目注意：在本 STM32H743 飞控项目中，MCP2515 位于独立 SPI2 上，STM32duino 的 `SPI.usingInterrupt()` 为空实现。为避免 ISR 中操作 SPI2 影响实时性，本项目不要使用 `CAN.onReceive()`，应在主循环/任务中轮询 `CAN.parsePacket()`。
