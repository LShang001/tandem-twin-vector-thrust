...
int MCP2515Class::endPacket()
{
  ...
  // 发送超时保护: 500kbps 下单帧约 200us, 这里给 1000us 绝对时间预算
  // 避免总线无 ACK 时无限阻塞 200Hz 调度任务
  ...
  while (readRegister(REG_TXBnCTRL(n)) & 0x08) {
    ...
    if ((uint32_t)(micros() - txStartUs) > 1000U) {
      // 超时放弃, 防止无限阻塞飞控调度
      aborted = true;
      modifyRegister(REG_CANCTRL, 0x10, 0x10);
      break;
  ...
  if (aborted) {
    // clear abort command，并短暂等待 TXREQ 清除，避免 TXB0 状态残留
    const uint32_t abortStartUs = micros();
    while ((readRegister(REG_TXBnCTRL(n)) & 0x08) &&
           ((uint32_t)(micros() - abortStartUs) <= 200U)) {
      yield();
    }
...
int MCP2515Class::parsePacket()
{
  ...
  if (_rxDlc > 8) _rxDlc = 8;  // 钳制 DLC 到硬件缓冲区上限 (纵深防御)
  _rxIndex = 0;
...
void MCP2515Class::setSPI(SPIClass& spi)
{
  _spi = &spi;
}
