 ...
 * @brief 获取FIFO值
 *
 * 本函数从设备的FIFO中读取一个24位的有符号整数，并将其值通过引用参数返回
 * 函数首先检查输入参数是否为NULL，然后从特定的寄存器块中读取数据如果读取操作失败或输入参数无效，
 * 函数将返回一个错误代码读取成功后，函数将从缓冲区中提取24位值，并将其转换为二进制补码形式返回
 *
 * @param value 指向一个int32_t变量的指针，用于存储从FIFO中读取的值如果读取失败，此变量保持不变
 * @return int16_t 返回一个表示操作结果的16位有符号整数，通常是一个状态代码
 *
 * @retval DPS__FAIL_UNKNOWN 如果输入参数为NULL或读取操作失败
 * @retval 0x01 如果读取成功且24位值的最低位为1
 * @retval 0x00 如果读取成功且24位值的最低位为0
 */
int16_t DpsClass::getFIFOvalue(int32_t *value)
{
	// 初始化一个缓冲区，用于存储从设备读取的数据
	uint8_t buffer[DPS__RESULT_BLOCK_LENGTH] = {0};

	// 检查输入参数是否为NULL，并尝试从寄存器块中读取数据
	// 如果输入参数无效或读取操作失败，返回错误代码
	if (value == NULL || readBlock(registerBlocks[PRS], buffer) != DPS__RESULT_BLOCK_LENGTH)
		return DPS__FAIL_UNKNOWN;

	// 从缓冲区中提取24位值，并将其转换为一个32位有符号整数
