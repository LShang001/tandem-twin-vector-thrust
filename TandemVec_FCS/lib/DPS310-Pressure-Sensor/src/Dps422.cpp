...
int16_t Dps422::measureBothOnce(float &prs, float &temp, uint8_t prs_osr, uint8_t temp_osr)
{
	...
	{
		// 温度过采样率变化时必须配置温度通道，不能误写到压力通道。
		if (configTemp(0U, temp_osr))
		{
			return DPS__FAIL_UNKNOWN;
		}
	}
	...
	// 理论上 ready 标志只会落在上述分支，保留默认失败返回防止异常位型穿透。
	return DPS__FAIL_UNKNOWN;
}
...
int16_t Dps422::readcoeffs(void)
{
	...
	// 校准系数读取失败时不能继续解析未初始化缓冲区。
	if (readBlock(coeffBlocks[COEF_TEMP], buffer_temp) != coeffBlocks[COEF_TEMP].length ||
		readBlock(coeffBlocks[COEF_PRS], buffer_prs) != coeffBlocks[COEF_PRS].length)
	{
		return DPS__FAIL_UNKNOWN;
	}
