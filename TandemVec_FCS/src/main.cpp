...
void setup()
{
  ...
  //   速率环 kp 的数值单位为 s^-1，可直接作为理想内环带宽；
  //   外环ωn²=Kp_r×Kp_a,  ζ=Kp_r/(2ωn)
  //
  // Pitch/Roll: Kp_r=16.042818s^-1, Kp_a=2.8（现役参数见 FlightCtrlParams.h）
  // Yaw(差速):  Kp_r=11.459156s^-1, Kp_a=5.0；电机 τm=0.28s 由观测器补偿
  //
  // 速率 PID 输出/积分限幅采用 deg/s^2；进入 ControlOutputs_t 前唯一转换为 rad/s^2。
  // ====================================================================
