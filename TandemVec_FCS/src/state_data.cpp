...
//    带宽必须 ≪1/τm=3.57 → 取 ωn≈2.1, ζ≈1.3（过阻尼无超调）:
//    Kp_r=0.10 → 2ζωn=5.73; Kp_a=0.8 → ωn=√(5.73×0.8)=2.14, ζ=1.34
//  TVC轴(pitch尾摆/yaw前摆): 舵机快(333Hz)、无电机滞后，可提高带宽补阻尼，
//    尾摆 Kp_r=0.25 → 2ζωn=14.3; Kp_a=2.5 → ωn=√(14.3×2.5)=5.98, ζ=1.20
//    前摆 Kp_r=0.22 → 2ζωn=12.6; Kp_a=2.0 → ωn=√(12.6×2.0)=5.02, ζ=1.26
// 2026-08-07 第三轮（用户：角速度环可调大）：内环 P 上调 43~69%，外环不变
//   → ωn 升、ζ 同步升（内环 P 增大即增阻尼），响应更有力且更不易超调
PositionPID rollAnglePID(0.8f, 0.0f, 0.0f);   // 差速外环：过阻尼设计 ζ≈1.34
PositionPID rollRatePID(0.10f, 0.0003f, 0.0f); // 差速内环 0.07→0.10（受 τm=0.28s 限带，保守）
PositionPID pitchAnglePID(2.5f, 0.0f, 0.0f);  // 尾摆外环：提高带宽
PositionPID pitchRatePID(0.25f, 0.0003f, 0.0f); // 尾摆内环 0.15→0.25（ζ≈1.20）
PositionPID yawAnglePID(2.0f, 0.0f, 0.0f);   // 前摆外环：略保守于俯仰
PositionPID yawRatePID(0.22f, 0.0003f, 0.0f); // 前摆内环 0.13→0.22（ζ≈1.26）
