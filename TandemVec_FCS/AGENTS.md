...
- **AUTO_POSITION/GUIDED 目标姿态构造已修复**（2026-08-02）：`q_yaw_base` 从"绕机体 z 航向"（多旋翼式假设）改为 VTOL 悬停基准合成 `q_hover ⊗ Rx(-Heading)`（q_hover = 绕 NED y 转 90° 机头朝天；悬停时航向轴 = 机体 x = 差速轴）。数值验证：悬停+航向 90° 目标姿态精确 = 绕 NED z 转 90°（fca T8 断言锁定；`scripts/verify_att_target.py` 推导可复现）。`constructTiltTargetQuaternion` 本身公式不变（NED 水平轴纯倾斜，已验证）。
