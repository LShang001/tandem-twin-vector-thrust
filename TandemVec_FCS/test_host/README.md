...
| `test_flight_control_axis.cpp` | `flight_control.cpp` 姿态环闭环仿真（轴序 FRD 修复验证）：roll←q_err.x/ω.x、pitch←q_err.y/ω.y、yaw←ω.z；刚体角动力学 + 推进力矩闭环收敛、执行器符号与 propulsion.mjs 交叉一致 | 无 | 11 |
