...
| `test_flight_control_axis.cpp` | `flight_control.cpp` 姿态环闭环仿真（轴序 FRD/VTOL 悬停修复验证）：roll←q_err.x/ω.x、pitch←q_err.y/ω.y、yaw←q_err.z/ω.z；刚体角动力学 + 推进力矩闭环收敛、执行器符号与 propulsion.mjs 交叉一致、推力垂直投影 R13（cos_tilt）、目标姿态合成（q_hover ⊗ Rx(-Heading)）、四轴式摇杆映射、六自由度悬停全动力学（配平平衡/航向保持/饱和/电机滞后/陀螺耦合/航向指令跟踪/保范） | 无 | 38 |
