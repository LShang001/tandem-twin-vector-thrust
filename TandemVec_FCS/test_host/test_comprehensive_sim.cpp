                      ...
                      float out[4])
{
  ...
  rate.setIntegralLimit(ControlUnits::radps2ToDps2(10.f));rate.setIntegralThreshold(30.f);
  ...
  for(int i=0;i<N;i++){
    ...
    // Inner P+I: deg/s → deg/s²；物理边界转 rad/s²。
    const float alpha_dps2=rF.filter(rate.computeDerivativeOnMeasurement(wref,gf_p, 0.005f));
    const float alpha_radps2=ControlUnits::dps2ToRadps2(alpha_dps2);

    // I×α → M_cmd
    float My_cmd=P.Iy*alpha_radps2;
...
int main()
{
  ...
  for(float r=0.05f;r<=0.55f;r+=0.05f)std::printf(" %6.2f",r*old_to_angle);
  std::printf("\n");
  for(float a=1.f;a<=8.f;a+=1.f){
    std::printf(" %4.0f   ",a);
    for(float r=0.05f;r<=0.55f;r+=0.05f){
      sim_pitch(a,r*old_to_angle,0.06f*old_to_angle,thr,3.f,3.f,tgt,4.f,dt,o);
  ...
  // ═══ S2: 精细扫描（含扰动） ═══
  std::printf("── S2: 精细扫描 Kp_a=1.5~4.0, Kp_r=4.58~14.32 s^-1（含扰动） ──\n");
  ...
  for(float a:{1.5f,2.f,2.5f,3.f,3.5f,4.f}){
    for(float r:{0.08f,0.10f,0.12f,0.15f,0.18f,0.20f,0.25f}){
      sim_pitch(a,r*old_to_angle,0.06f*old_to_angle,thr,3.f,3.f,tgt,sim,dt,o);
      std::printf("%7.1f %7.2f %7.1f %7.2f %7.2f\n",a,r*old_to_angle,o[0],o[1],o[2]);
      ++s2_n;
  ...
  // ═══ S3: 扰动 & 鲁棒性 ═══
  // 当前配置直接读取唯一参数源，避免研究扫描点被误标为现役增益。
  const float current_a=kFlightCtrlParams.att_pitch.kp;
  const float current_r=kFlightCtrlParams.rate_pitch.kp;
  const float current_ki=kFlightCtrlParams.rate_pitch.ki;
  std::printf("\n── S3: 扰动&鲁棒性 (现役 Kp_a=%.1f Kp_r=%.2f Ki=%.2f) ──\n",
              current_a,current_r,current_ki);
  std::printf("%-20s %8s %8s %8s\n","工况","peak°","settle","final°");

  float s3_ideal_final,s3_dist_final; float s3_ideal_settle,s3_dist_settle;
  sim_pitch(current_a,current_r,current_ki,thr,0.f,0.f,tgt,sim,dt,o);
  std::printf("%-20s %7.1f %7.2f %7.2f\n","理想(无扰动)",o[0],o[1],o[2]);
  s3_ideal_final=o[2]; s3_ideal_settle=o[1];

  sim_pitch(current_a,current_r,current_ki,thr,5.f,5.f,tgt,sim,dt,o);
  ...
  // C1: 精细扫描区内的误差与超调有界性。该单轴台架未闭环抑制
  // 反扭耦合激发的其他轴；长时浮点差异会被非线性分配放大，因此不将“8 s 内
  // 进入 ±0.4° 窄带”当作整个增益域的硬回归条件。窄带计数仅作信息输出。
  check(s2_worst_final < 3.2f,
        "C1 精细扫描最大末值误差 < 3.2°（全域有界）");
  check(s2_worst_ov < 0.35f,
        "C1 精细扫描最大超调 < 35%");
  ...
  //   常值扰动下若未进入窄带，不等同于发散；这里直接约束末值误差上界。
  ...
  std::printf("═══════════════════════════════════════════\n");
  std::printf("现役对照: Kp_a=%.1f, Kp_r=%.2f, Ki=%.2f（S3 理想工况）\n",
              current_a,current_r,current_ki);
  std::printf("扫描结论: Kp_a=1.5~4.0, Kp_r=4.58~14.32 s^-1 在常值扰动下\n");
  std::printf("          保持有界，但不是所有组合都在 8s 内进入±2%%窄带。\n");
  std::printf("实飞步骤: 从固件当前值起飞，若响应偏软增大 Kp_r（每次约+2.86 s^-1），\n");
